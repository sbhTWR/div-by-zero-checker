...
public class DivByZeroVisitor extends BaseTypeVisitor<DivByZeroAnnotatedTypeFactory> {
  ...
  private boolean errorAt(BinaryTree node) {
    ...
    // an "Undefined" type qualifier is assigned when a divide-by-zero is possible.
    // it will be caught during assignment statement below, so no need to catch here!  
    return false;
  }
  ...
  private boolean errorAt(CompoundAssignmentTree node) {
    ...
    if (hasAnnotation(rhs, Undefined.class)) {
      return true;
    }
